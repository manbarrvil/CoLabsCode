...
from State_Estimation_meas_process_rt import process_power_system_data
from observability_check import observability_check
from State_Estimation_execute_simple_rt import State_Estimation_execute
from residuals_simple import residuals

def State_Estimation_generic_function(num_buses, slack_bus,der_positions_ratings, zero_injection_buses, hashmap_reduction2, hashmap_names2, H_v_full, H_p_full, H_q_full, slack_bus_row_P, slack_bus_row_Q, injection_buses, Pmes, Qmes, Vmes,adjacency_matrix_buses, adjacency_matrix_branches, parent_child_adjacency, R, X, Bshunt, Gshunt, S_n):

    mes_positions_p, mes_positions_q, mes_positions_v, mes_positions_pflow, mes_positions_qflow, Pmes, Qmes, Vmes, Pflow_mes, Qflow_mes =process_power_system_data(Pmes, Qmes, Vmes, num_buses, der_positions_ratings, slack_bus, injection_buses)
    ...
    if column_rank == (num_buses - 1) * 2:
        Vest, Pest, Qest, Vmesp, Pmesp, Qmesp, Std_V, Std_P, Std_Q, mes_v, mes_p, mes_q = State_Estimation_execute(num_buses, mes_positions_p, mes_positions_q, mes_positions_v, mes_positions_pflow, mes_positions_qflow, zero_injection_buses, Pmes, Qmes, Vmes, Pflow_mes, Qflow_mes, slack_bus, std_vector_p, std_vector_q, std_vector_v,adjacency_matrix_buses, adjacency_matrix_branches, parent_child_adjacency, R, X, Bshunt, Gshunt, S_n)
